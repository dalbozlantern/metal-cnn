...
def fourier_image(greyscale_image):
    fourier = np.fft.fft2(greyscale_image)
    freq_shifted = np.fft.fftshift(fourier)
    return np.dstack((freq_shifted.real, freq_shifted.imag))

def fourier_reverse(stacked_transform):
    real = np.take(stacked_transform, 0, 2)
    imaginary = np.take(stacked_transform, 1, 2)
    un_stacked = real + 1j*imaginary
    unshifted = np.fft.ifftshift(un_stacked)
    return np.abs(np.fft.ifft2(unshifted))
...
for depth in range(NUM_LAYERS - 1, -1, -1):
    for epoch_i in range(N_EPOCHS):
        for batch_i in range(1): #TODO
            ...
            batch_xs_f = [fourier_image(batch_xs_f[i]) for i in range(len(batch_xs))]
            batch_xs_f = np.reshape(batch_xs_f, (BATCH_SIZE, IMAGE_DIM * IMAGE_DIM * 2))
            train = batch_xs_f
    ...
    test_xs_f = [fourier_image(test_xs_f[i]) for i in range(len(test_xs_f))]
    test_xs_f = np.reshape(test_xs_f, (BATCH_SIZE, IMAGE_DIM * IMAGE_DIM * 2))

test_xs_norm = test_xs_f #new
...
for i in range(5):
    ...
    plt.imshow(fourier_reverse(test_xs_f[i].reshape(IMAGE_DIM, IMAGE_DIM, CHANNELS)), vmin=0, vmax=1, cmap="gray")
    plt.title("Test input")
    plt.colorbar()
    plt.subplot(5, 2, 2*i + 2)
    plt.imshow(fourier_reverse(recon[i].reshape(IMAGE_DIM, IMAGE_DIM, CHANNELS)), vmin=0, vmax=1, cmap="gray")
