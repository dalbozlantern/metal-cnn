...
def decoder_layer(x, in_depth, out_depth, in_height, out_height):
    ...
    bias = initialize_bias_variable([out_depth])
    hidden_step_1 = tf.nn.conv2d_transpose(hidden_step_0, filter=weights,
                                           output_shape=[-1, out_height, out_height, out_depth],
                                           strides=[1, 1, 1, 1], padding='SAME')
    return tf.nn.relu(hidden_step_1 + bias)


layer_1_out = encoder_layer(x_image, 1, 64, pool=False)  # Conv: Input ([256 x 256 x 1]) --> [256 x 256 x 64]
...
layer_15_out = decoder_layer(layer_14_out, 128, 64, 128, 256)  # Conv: [256 x 256 x 64] <-- [128 x 128 x 128]
reconstruction = decoder_layer(layer_15_out, 64, 1, 256, 256)  # Conv: Output ([256 x 256 x 1]) <-- [256 x 256 x 64]
cost = tf.reduce_mean(tf.nn.l2_loss(tf.subtract(x_image, reconstruction)))
